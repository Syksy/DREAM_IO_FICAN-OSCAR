...
aggregateX <- function(
	...
	# CD8A expression level
	X <- cbind(X, CD8A = logz(gex["CD8A",]))
...
#gex_input <- gex_synthetic[order(rownames(gex_synthetic)),]
#gex_input <- as.matrix(gex_synthetic)
...
# Sanity checking
#head(cbind(ret, dat_input[,c("TMB", "PDL1", "SEX")], t(gex_input[c("CD274","CD8A"),])))
